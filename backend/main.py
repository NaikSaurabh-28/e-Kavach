...
# Ensure existing frontend isn't broken by still supporting the /upload route
@app.post("/upload")
...
async def sanitize_file(file: UploadFile = File(...)):
    if not file.filename.lower().endswith('.pdf'):
        return {"success": False, "message": "Only PDF files can be sanitized."}
        
    try:
        content = await file.read()
        result = sanitize_pdf(content, file.filename)
        return result
    except Exception as e:
...
async def download_sanitized(filename: str):
    file_path = os.path.join("sanitized", filename)
    if os.path.exists(file_path):
        # Ensure the served filename always has .pdf extension
        display_name = filename if filename.lower().endswith('.pdf') else f"{filename}.pdf"
        return FileResponse(
            file_path, 
